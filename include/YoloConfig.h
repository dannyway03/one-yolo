...
// "ground track field", "harbor", "bridge", "large vehicle", "small vehicle", "helicopter", "roundabout", "soccer ball
// field", "swimming pool"}
#define COCO_NAMES                                                                                                     \
...
struct YoloConfig
{
  ...
  float iou_thresh_ = 0.45f;   // iou threshold of postprocess (ignored if nms-free) for Yolo model, 0.45f by default
  float approx_f_ = 0.001f;    // approximation accuracy factor of postprocess for Yolo model (calculate contour in
                               // segmentation task), 0.001f by default
  float scale_f_ = 1.0f / 255; // scale factor of preprocess for Yolo model, 1.0f / 255 by default
  bool nchw_ = true;           // channel-first or not for Yolo model, true by default
  bool rgb_ = true;            // order of r,g,b values for Yolo model, true by default
