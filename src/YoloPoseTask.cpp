...
YoloPoseTask::~YoloPoseTask() = default;

void
YoloPoseTask::collect_boxes_yolo5u_8_11(const cv::Mat& output, int batch_id, const cv::Size& orig_size,
                                        const LetterBoxInfo& lb, std::vector<cv::Rect>& boxes,
                                        std::vector<float>& scores, std::vector<int>& cls_ids,
                                        std::vector<std::vector<YoloKeyPoint>>& kpts)
{
  ...
  const int num_classes = elem_dim - 4 - _cfg.num_kpts_ * 3;
  ...
  for (int i = 0; i < num_preds; ++i)
  {
    ...
    for (size_t j = 0; j < _cfg.num_kpts_; j++)
    {
...
void
YoloPoseTask::collect_boxes_yolo26(const cv::Mat& output, int batch_id, const cv::Size& orig_size,
                                   const LetterBoxInfo& lb, std::vector<cv::Rect>& boxes, std::vector<float>& scores,
                                   std::vector<int>& cls_ids, std::vector<std::vector<YoloKeyPoint>>& kpts)
{
  int num_preds = 0;
  // const value
  const int elem_dim = 6 + _cfg.num_kpts_ * 3;
  const float* data = nullptr;

  if (output.dims == 3)
  {
    // [batch, num_preds, 6 + _cfg.num_kpts_ * 3]
    // [batch, num_preds, (x1, y1, x2, y2, conf, cls_id, _cfg.num_kpts_ * 3)]
  ...
  {
    // [num_preds, 6 + _cfg.num_kpts_ * 3]
    // [num_preds, (x1, y1, x2, y2, conf, cls_id, 6 + _cfg.num_kpts_ * 3)]
...
void
YoloPoseTask::postprocess_one(const std::vector<cv::Mat>& raw_outputs, int batch_id, cv::Size orig_size,
                              LetterBoxInfo lb_info, YoloResult& result)
{
  ...
  {
    // anchor-free
    collect_boxes_yolo5u_8_11(output, batch_id, orig_size, lb_info, tmp_boxes, tmp_scores, tmp_cls_ids, tmp_kpts);
  ...
  else if (_cfg.version_ == YoloVersion::YOLO26)
  {
    ...
    collect_boxes_yolo26(output, batch_id, orig_size, lb_info, tmp_boxes, tmp_scores, tmp_cls_ids, tmp_kpts);
  ...
  {
    YoloPoseObj pose_obj{boxes[i], cls_ids[i], scores[i], _cfg.names_[cls_ids[i]], kpts[i]};
