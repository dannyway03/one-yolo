...
YoloSegTask::~YoloSegTask() = default;

auto
YoloSegTask::sigmoid(const cv::Mat& x) -> cv::Mat
{
  cv::Mat result;
  cv::exp(-x, result);
...
auto
YoloSegTask::process_mask_one(const cv::Mat& protos, // [batch, _cfg.num_channels_, _cfg.input_h_/4, _cfg.input_w_/4]
                              int batch_id,
                              const cv::Mat& coeff, // [1, _cfg.num_channels_]
                              const cv::Rect& bbox, const cv::Size& input_size, const cv::Point& padding,
                              const float x_scale, const float y_scale) -> cv::Mat
{
  // get proto by batch id
  auto un_const_protos = const_cast<cv::Mat&>(protos);
  auto proto_ptr = un_const_protos.ptr<float>(batch_id);
  std::array<int, 3> proto_s = {protos.size[1], protos.size[2], protos.size[3]};
  // [_cfg.num_channels_, _cfg.input_h_/4, _cfg.input_w_/4]
  cv::Mat proto(3, proto_s.data(), CV_32F, proto_ptr);

  auto h_proto = proto.size[1];
  auto w_proto = proto.size[2];

  // [_cfg.num_channels_, _cfg.input_h_/4 * _cfg.input_w_/4]
  cv::Mat proto_flat = proto.reshape(1, _cfg.num_channels_);

  // [1,_cfg.num_channels_] × [_cfg.num_channels_, _cfg.input_h_/4 * _cfg.input_w_/4] → [1, _cfg.input_h_/4 *
  // _cfg.input_w_/4]
  cv::Mat mask_raw = coeff * proto_flat;

  // [_cfg.input_h_/4, _cfg.input_w_/4]
...
void
YoloSegTask::collect_boxes_yolo5(const cv::Mat& output0, const cv::Mat& output1, int batch_id,
                                 const cv::Size& orig_size, const LetterBoxInfo& lb, std::vector<cv::Rect>& boxes,
                                 std::vector<cv::Mat>& masks, std::vector<float>& scores, std::vector<int>& cls_ids)
{
  // output0: [batch, num_preds, 4 + obj_conf + num_classes + _cfg.num_channels_]
  // output1: [batch, _cfg.num_channels_, _cfg.input_h_/4, _cfg.input_w_/4]
  int num_preds = 0;
  int elem_dim = 0;
  ...
  // [cx, cy, w, h, obj_conf, num_classes, _cfg.num_channels_]
  const int num_classes = elem_dim - 5 - _cfg.num_channels_;
  ...
  for (int i = 0; i < num_preds; ++i)
  {
    ...
    if (conf < _cfg.conf_thresh_)
      continue;

    cv::Rect box = utils.decode_box(cx, cy, w, h, lb, orig_size);
    ...
    cv::Mat coeff(1, _cfg.num_channels_, CV_32F);
    for (int m = 0; m < _cfg.num_channels_; ++m)
...
void
YoloSegTask::collect_boxes_yolo5u_8_11(const cv::Mat& output0, const cv::Mat& output1, int batch_id,
                                       const cv::Size& orig_size, const LetterBoxInfo& lb, std::vector<cv::Rect>& boxes,
                                       std::vector<cv::Mat>& masks, std::vector<float>& scores,
                                       std::vector<int>& cls_ids)
{
  // output0: [batch, 4 + num_classes + _cfg.num_channels_, num_preds]
  // output1: [batch, _cfg.num_channels_, _cfg.input_h_/4, _cfg.input_w_/4]
  int num_preds = 0;
  int elem_dim = 0;
  const float* data = nullptr;
  ...
  // [cx, cy, w, h, num_classes, _cfg.num_channels_]
  const int num_classes = elem_dim - 4 - _cfg.num_channels_;
  assert(num_classes == _cfg.num_classes_);
  YoloUtils utils;
  ...
  for (int i = 0; i < num_preds; ++i)
  {
    ...
    if (best_score < _cfg.conf_thresh_)
      continue;
...
void
YoloSegTask::collect_boxes_yolo26(const cv::Mat& output0, const cv::Mat& output1, int batch_id,
                                  const cv::Size& orig_size, const LetterBoxInfo& lb, std::vector<cv::Rect>& boxes,
                                  std::vector<cv::Mat>& masks, std::vector<float>& scores, std::vector<int>& cls_ids)
{
  // output0: [batch, num_preds, 6 + _cfg.num_channels_]
  // output1: [batch, _cfg.num_channels_, _cfg.input_h_/4, _cfg.input_w_/4]
  int num_preds = 0;
  // const value
  const int elem_dim = 6 + _cfg.num_channels_;
  const float* data = nullptr;

  if (output0.dims == 3 && output1.dims == 4)
  {
    // [batch, num_preds, 6 + _cfg.num_channels_]
    // [batch, num_preds, (x1, y1, x2, y2, conf, cls_id) + _cfg.num_channels_]
    assert(batch_id < output0.size[0]);
    assert(batch_id < output1.size[0]);
    num_preds = output0.size[1];
    data = output0.ptr<float>(batch_id);
  ...
  for (int i = 0; i < num_preds; ++i)
  {
    ...
    if (cls < 0 || cls >= _cfg.num_classes_)
      continue;

    cv::Rect box = utils.decode_box((x1 + x2) / 2, (y1 + y2) / 2, x2 - x1, y2 - y1, lb, orig_size);
...
void
YoloSegTask::postprocess_one(const std::vector<cv::Mat>& raw_outputs, int batch_id, cv::Size orig_size,
                             LetterBoxInfo lb_info, YoloResult& result)
{
  ...
  const cv::Mat& output0 =
    raw_outputs[0].dims == 3 ? raw_outputs[0] : raw_outputs[1]; // output0: depends on YoloVersion
  const cv::Mat& output1 = raw_outputs[1].dims == 4 ?
                             raw_outputs[1] :
                             raw_outputs[0]; // output1: [batch, _cfg.num_channels_, _cfg.input_h_/4, _cfg.input_w_/4]
  ...
  YoloUtils utils;
  if (_cfg.version_ == YoloVersion::YOLO5)
  {
    // anchor-based
    collect_boxes_yolo5(output0, output1, batch_id, orig_size, lb_info, tmp_boxes, tmp_masks, tmp_scores, tmp_cls_ids);
    utils.class_aware_nms(tmp_boxes, tmp_scores, tmp_cls_ids, _cfg.conf_thresh_, _cfg.iou_thresh_, keep_indices);
  }
  else if (_cfg.version_ == YoloVersion::YOLO5U || _cfg.version_ == YoloVersion::YOLO8 ||
           _cfg.version_ == YoloVersion::YOLO11)
  {
    // anchor-free
    collect_boxes_yolo5u_8_11(output0, output1, batch_id, orig_size, lb_info, tmp_boxes, tmp_masks, tmp_scores,
                              tmp_cls_ids);
    utils.class_aware_nms(tmp_boxes, tmp_scores, tmp_cls_ids, _cfg.conf_thresh_, _cfg.iou_thresh_, keep_indices);
  }
  else if (_cfg.version_ == YoloVersion::YOLO26)
  {
    // anchor-free & nms-free
    collect_boxes_yolo26(output0, output1, batch_id, orig_size, lb_info, tmp_boxes, tmp_masks, tmp_scores, tmp_cls_ids);
  ...
  for (int idx : keep_indices)
  {
    ...
    auto& protos = output1;
    auto local_mask = process_mask_one(protos, batch_id, coeff, tmp_boxes[idx], cv::Size(_cfg.input_w_, _cfg.input_h_),
  ...
  for (size_t i = 0; i < keep_indices.size(); i++)
  {
    ...
    if (!contours.empty())
    {
      ...
      const auto& largest_contour = *max_contour_it;

      // reduce the points to make contour more simple
      double epsilon = _cfg.approx_f_ * cv::arcLength(largest_contour, true);
    ...
    */
    YoloSegObj seg_obj{boxes[i], cls_ids[i], scores[i], _cfg.names_[cls_ids[i]], mask, approx_contour};
