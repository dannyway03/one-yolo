                              ...
                              const float x_scale, const float y_scale) -> cv::Mat
{
  ...
  // [1,_cfg.num_channels_] × [_cfg.num_channels_, _cfg.input_h_/4 * _cfg.input_w_/4] → [1, _cfg.input_h_/4 * _cfg.input_w_/4]
  ...
  return mask_bin;
    ...
    void YoloSegTask::collect_boxes_yolo5u_8_11(
        ...
        for (int i = 0; i < num_preds; ++i) {
            ...
            if (best_score < _cfg.conf_thresh_) continue;
    ...
    void YoloSegTask::postprocess_one(
        ...
        const cv::Mat& output1 = raw_outputs[1].dims == 4 ? raw_outputs[1] : raw_outputs[0];  // output1: [batch, _cfg.num_channels_, _cfg.input_h_/4, _cfg.input_w_/4]
        ...
        else if (_cfg.version_ == YoloVersion::YOLO5U
        || _cfg.version_ == YoloVersion::YOLO8 
        ...
        for (int idx : keep_indices) {
            ...
            auto local_mask = 
                process_mask_one(
                    ...
                    cv::Size(_cfg.input_w_, _cfg.input_h_),
