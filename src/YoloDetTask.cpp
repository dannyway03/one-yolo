...
void
YoloDetTask::collect_boxes_yolo5(const cv::Mat& output, int batch_id, const cv::Size& orig_size,
                                 ...
                                 std::vector<int>& cls_ids)
{
  ...
  for (int i = 0; i < num_preds; ++i)
  {
    ...
    cv::Rect box = utils.decode_box(ptr[0], ptr[1], ptr[2], ptr[3], lb, orig_size);
...
void
YoloDetTask::collect_boxes_yolo5u_8_11(const cv::Mat& output, int batch_id, const cv::Size& orig_size,
...
void
YoloDetTask::collect_boxes_yolo26(const cv::Mat& output, int batch_id, const cv::Size& orig_size,
                                  const LetterBoxInfo& lb, std::vector<cv::Rect>& boxes, std::vector<float>& scores,
                                  std::vector<int>& cls_ids)
{
  int num_preds = 0;
...
void
YoloDetTask::postprocess_one(const std::vector<cv::Mat>& raw_outputs, int batch_id, cv::Size orig_size,
                             LetterBoxInfo lb_info, YoloResult& result)
{
  ...
  {
    // anchor-based
    collect_boxes_yolo5(output, batch_id, orig_size, lb_info, tmp_boxes, tmp_scores, tmp_cls_ids);
  ...
  {
    // anchor-free
    collect_boxes_yolo5u_8_11(output, batch_id, orig_size, lb_info, tmp_boxes, tmp_scores, tmp_cls_ids);
    utils.class_aware_nms(tmp_boxes, tmp_scores, tmp_cls_ids, _cfg.conf_thresh_, _cfg.iou_thresh_, keep_indices);
  }
  else if (_cfg.version_ == YoloVersion::YOLO26)
  {
    // anchor-free & nms-free
    collect_boxes_yolo26(output, batch_id, orig_size, lb_info, tmp_boxes, tmp_scores, tmp_cls_ids);
  ...
  {
    YoloDetObj det_obj{boxes[i], cls_ids[i], scores[i], _cfg.names_[cls_ids[i]]};
